...
BEGS :cite:`BEGS1` call :math:`{\mathcal X}_t` the **effective** government deficit and :math:`{\mathcal B}_t` the **effective** government debt.
...
where the dependence on :math:`\tau` is meant to remind us that these objects depend on the tax rate;  :math:`s_{-}` is last period's Markov state.
...
The topic involves interpreting  and extending formula :eq:`eq_criterion_fiscal_1` for the ergodic mean :math:`{\mathcal B}^*`.
...
Notice how attributes of the ergodic distribution for :math:`{\mathcal B}_t`  appear
on the right side of  formula :eq:`eq_criterion_fiscal_1` for approximating  the ergodic mean via :math:`{\mathcal B}^*`.
...
BEGS also :cite:`BEGS1` propose  an approximation to  :math:`{\mathcal B}^*` that can be computed **without** first approximating  the
...
We'll use a Python root solver to find a :math:`\tau` that solves this
...
We put :math:`{\rm var}(J({\mathcal B}))` into a Python function minimizer and
compute
...
:math:`E_t u_{c,t+1} = E u_{c,t+1}` in the ergodic distribution.
...
*  :math:`\hat b` is represented by the red vertical line plotted in the histogram of the last 100,000 observations of our simulation of the  par value of government debt plotted above
