...
As with the `ordinary LQ control model <https://python-intro.quantecon.org/lqcontrol.html>`__, :math:`J` takes the form :math:`J(x) = x' P x` for some symmetric positive definite matrix :math:`P`.
...
First, using `matrix calculus <https://python-intro.quantecon.org/linear_algebra.html#la-mcalc>`__, you will be able to verify that
...
Let us consider a monopolist similar to `this one <https://python-intro.quantecon.org/lqcontrol.html#lqc-mwac>`__, but now facing model uncertainty.
